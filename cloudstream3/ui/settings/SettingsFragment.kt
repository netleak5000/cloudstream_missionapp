...
import com.lagradost.cloudstream3.AuthManager
import com.lagradost.cloudstream3.BuildConfig
import com.lagradost.cloudstream3.PluginResetManager
...
class SettingsFragment : BaseFragment<MainSettingsBinding>(
    ...
    companion object {
        ...
        /*ankit_open*/
        const val ACTION_PLUGINS_RESET = "com.lagradost.cloudstream3.ACTION_PLUGINS_RESET"
        const val EXTRA_RESET_SUCCESS = "com.lagradost.cloudstream3.EXTRA_RESET_SUCCESS"
        /*ankit_closed*/
    ...
    override fun onBindingCreated(binding: MainSettingsBinding) {
        ...
        binding?.btnLogoutSettings?.setOnClickListener {
            AuthManager.logoutUser(requireActivity())
        }

        // VPN Button Action
        binding?.btnVpnSettings?.setOnClickListener {
            openProtonVPN()
        }

        binding?.btnResetPlugins?.setOnClickListener {
            // Show confirmation dialog and run reset
            PluginResetManager.showResetDialog(requireContext()) { success ->
                // Send a broadcast so MainActivity (if running) refreshes UI and clears in-memory caches
                try {
                    val intent = Intent(ACTION_PLUGINS_RESET)
                    intent.putExtra(EXTRA_RESET_SUCCESS, success)
    ...
    private fun openProtonVPN() {
        try {
            val intent = requireContext().packageManager.getLaunchIntentForPackage("ch.protonvpn.android")
            if (intent != null) {
                requireContext().startActivity(intent)
            } else {
                // Redirect to Play Store if ProtonVPN is not installed
                val playStoreIntent = Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=ch.protonvpn.android"))
